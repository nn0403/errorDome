...
export default store => next => action => {
    ...
    if (data) {
        return fetchDataPost(endpoint, data, schema).then(
        ...
        return fetchDataGet(endpoint, schema).then(
            response => next(actionWith({
                type: successType,
                response
            })),
...
//执行网络请求
const fetchDataGet = (endpoint, schema) => {
    return get(endpoint).then(data => {
        return normalizeData(data, schema)
    })
};

const fetchDataPost = (endpoint, data, schema) => {
    return post(endpoint, data).then(data => {
        return normalizeData(data, schema)
    })
};
